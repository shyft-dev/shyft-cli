...
featuresCommand.command("link-pr [id]").description("Link a pull request to a feature").requiredOption("--url <url>", "Full URL of the PR (e.g. https://github.com/owner/repo/pull/42)").option("--number <n>", "PR number (parsed from --url if omitted)").option("--repo <owner/repo>", "Repository full name (parsed from --url if omitted)").action(async (id, opts) => {
  const ctx = getContextManager();
  let featureId;
  try {
    featureId = ctx.resolveFeatureId(id);
  ...
  if (number === void 0 || !repoFullName) {
    ...
    if (number === void 0) number = parsed.number;
...
program.name("shyft").description("CLI for the Shyft platform").version("0.4.9").option("--json", "Output in JSON format").hook("preAction", (thisCommand) => {
